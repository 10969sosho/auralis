@extends('layouts.app')
@section('title', 'Pembayaran Deportasi')

@section('content')
<div class="payment-page" style="max-width:680px;margin:0 auto;">
    <h1 style="font-size:24px;font-weight:700;margin-bottom:6px;">Pembayaran Deportasi</h1>
    <p style="color:#64748b;margin-bottom:20px;">Kod Tempahan: <strong>{{ $booking->booking_code }}</strong></p>
    ...
    <div style="background:#fff;border-radius:16px;padding:20px;box-shadow:0 1px 2px rgba(0,0,0,0.05);border:1px solid #e5e7eb;margin-bottom:20px;">
        <h2 style="font-size:15px;font-weight:700;margin-bottom:14px;">Ringkasan Tempahan</h2>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:14px;">
            ...
            <div style="color:#64748b;">Jadual</div>
            ...
            <div style="color:#64748b;">Penumpang</div>
            <div style="font-weight:600;">{{ $booking->total_passengers }} orang</div>
            ...
            <div style="border-top:1px solid #e5e7eb;padding-top:8px;font-weight:700;color:#1e293b;">Jumlah</div>
        ...
        <p style="margin-top:12px;font-size:13px;color:#EA580C;font-weight:600;">
            Sila selesaikan pembayaran sebelum: {{ $booking->expires_at->format('d M Y, H:i') }}
        </p>
    </div>
    ...
    @if($payment && $payment->payment_status === 'pending')
        <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:12px;padding:16px;margin-bottom:20px;text-align:center;">
            <p style="font-weight:700;color:#ea580c;">Pembayaran Belum Selesai</p>
            <p style="font-size:13px;color:#9a3412;margin-top:4px;">
                @if($payment->payment_method === 'toyibpay')
                    Sila selesaikan pembayaran melalui ToyibPay. Halaman akan dikemas kini secara automatik selepas pembayaran berjaya.
                @else
                    Sila muat naik bukti pembayaran di bawah.
                @endif
            </p>
        </div>
        ...
        <div style="background:#fff;border-radius:16px;padding:20px;box-shadow:0 1px 2px rgba(0,0,0,0.05);border:1px solid #e5e7eb;margin-bottom:20px;">
            <h3 style="font-weight:700;margin-bottom:12px;">Muat Naik Bukti Pemindahan</h3>
            <form action="{{ route('deportation.payment.process', $booking->booking_code) }}" method="POST" enctype="multipart/form-data">
                ...
                <div style="margin-bottom:12px;">
                    <label style="display:block;margin-bottom:6px;font-weight:600;font-size:13px;">Bukti Pemindahan (JPG/PNG/WEBP, maks 5MB) *</label>
    ...
    @elseif($payment && $payment->payment_status === 'awaiting_approval')
        <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:12px;padding:20px;text-align:center;">
            ...
            <p style="font-weight:700;color:#1e40af;font-size:16px;">Menunggu Pengesahan</p>
            <p style="font-size:13px;color:#3b82f6;margin-top:4px;">Bukti pembayaran anda sedang disemak oleh admin. Sila tunggu pengesahan.</p>
        </div>
    @elseif($payment && in_array($payment->payment_status, ['paid', 'approved']))
        <div style="background:#dcfce7;border:1px solid #bbf7d0;border-radius:12px;padding:20px;text-align:center;">
            <p style="font-weight:700;color:#16a34a;font-size:16px;">Pembayaran Berjaya!</p>
            <a href="{{ route('deportation.success', $booking->booking_code) }}" style="display:inline-block;margin-top:12px;background:#2563EB;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:600;">Lihat Tiket</a>
        </div>
    @endif

    {{-- Passengers List --}}
    <div style="background:#fff;border-radius:16px;padding:20px;box-shadow:0 1px 2px rgba(0,0,0,0.05);border:1px solid #e5e7eb;">
        <h3 style="font-weight:700;margin-bottom:12px;">Senarai Penumpang</h3>
            ...
            <table style="width:100%;border-collapse:collapse;">
                <thead>
                    <tr style="background:#f8fafc;font-size:12px;text-transform:uppercase;color:#64748b;">
                        ...
                        <th style="padding:10px;text-align:left;">Jantina</th>
                        <th style="padding:10px;text-align:left;">Kewarganegaraan</th>
