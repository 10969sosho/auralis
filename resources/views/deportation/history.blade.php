@extends('layouts.app')
@section('title', 'Sejarah Tempahan Deportasi')

@section('content')
<div class="deportation-page">
    <h1 style="font-size:24px;font-weight:700;color:#1e293b;margin-bottom:6px;">Sejarah Tempahan Deportasi</h1>
    ...
    @if($bookings->isEmpty())
        <div style="background:#fff;border-radius:16px;padding:60px 20px;text-align:center;box-shadow:0 1px 2px rgba(0,0,0,0.05);border:1px solid #e5e7eb;">
            <p style="color:#64748b;">Tiada sejarah tempahan deportasi.</p>
        </div>
    @else
            ...
            <div style="background:#fff;border-radius:16px;padding:20px;box-shadow:0 1px 2px rgba(0,0,0,0.05);border:1px solid #e5e7eb;">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;">
                    <div>
                        ...
                        <p style="font-size:13px;color:#64748b;">Jumlah: <strong>RM{{ number_format($booking->total_amount, 2) }}</strong></p>
                    </div>
                    <div style="text-align:right;">
                        ...
                        @if(in_array($booking->payment_status, ['paid', 'approved']))
                            <a href="{{ route('deportation.success', $booking->booking_code) }}" style="display:inline-block;margin-top:8px;color:#2563EB;text-decoration:none;font-weight:600;font-size:13px;">Lihat Tiket</a>
                        @elseif($booking->payment_status === 'pending')
                            <a href="{{ route('deportation.payment', $booking->booking_code) }}" style="display:inline-block;margin-top:8px;color:#ea580c;text-decoration:none;font-weight:600;font-size:13px;">Bayar</a>
                        @endif
                    </div>
                </div>
            </div>
